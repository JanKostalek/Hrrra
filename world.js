(function () {
  ...
  World.prototype.estimateMaxJumpRise = function () {
    var gravity = C.singleJumpGravity;
    var initialVelocity = C.singleJumpInitialVelocity;
    var holdAcceleration = C.singleJumpHoldAcceleration;
    var holdTime = C.singleJumpHoldMaxTime;

    var accelDuringHold = gravity - holdAcceleration;
    var riseHold = initialVelocity * holdTime - 0.5 * accelDuringHold * holdTime * holdTime;
    var velocityAfterHold = initialVelocity - accelDuringHold * holdTime;
    var riseAfterHold = (velocityAfterHold * velocityAfterHold) / (2 * gravity);
  ...
  World.prototype.tryGenerateStackedUpperPlatform = function (x, y, width) {
    ...
    var minGapMultiplier = Math.max(3, C.stackedGapMinPlayerHeightMultiplier);
    var minClearGap = C.playerSize * minGapMultiplier;
    var minGap = minClearGap + C.platformHeight;
    var maxGap = singleJumpRise * C.stackedGapMaxJumpMultiplier + C.platformHeight;
    var topLimitY = this.getPlatformYBounds().min;
    var feasibleMaxGap = y - topLimitY;

    if (feasibleMaxGap < minGap) {
      return;
    }

    var clampedMaxGap = Math.min(maxGap, feasibleMaxGap);
    if (clampedMaxGap < minGap) {
      return;
    }

    var gap = randomRange(minGap, clampedMaxGap);
    var upperY = y - gap;
    this.platforms.push(
