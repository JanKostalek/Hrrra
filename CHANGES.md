...
- 2026-04-01: On branch `highscore`, added the first online leaderboard foundation using `Vercel KV`: players now get a persistent local `player name` / `player id`, the first launch prompts for a name, `Game Over` can submit scores to a shared online board split by `Jump/Full` and `Easy/Hard`, and the result screen shows the top leaderboard plus the player's current best rank.
- 2026-04-02: Reworked the `Game Over` layout into three columns: `Your High Scores` on the left, the current run result centered with only `Score` and `Run Time`, and the online board on the right. The online panel now shows the top `15` scores for the currently played mode/difficulty plus always-appended `Your Best` and `Current Run` rows for that same board.
- 2026-04-02: Added mid-size responsive breakpoints for both `Game Over` and the start/pre-run screens so large desktop widgets shrink earlier, columns stack sooner, and neither screen should overflow or clip on medium-width laptop and emulator layouts.
- 2026-04-02: Refined the `Game Over` online panel so player rows now include rank prefixes and `Your Best` falls back to the stored local high score for the current board until the online leaderboard has fully responded, instead of showing `0` misleadingly.
- 2026-04-02: Replaced the left `Game Over` box with `Online High Scores - Top Scores` and kept the right box as `Online High Scores - Top Players`, both tied to the current played board (`Jump/Full` + `Easy/Hard`). The center run summary remains only `Score` and `Run Time`.
- 2026-04-02: Upgraded the online leaderboard API/UI contract to dual datasets: `Top Scores` now stores top run entries (same player can appear multiple times), while `Top Players` stores one best score per player. Added dedicated `bestScoreRank` / `currentScoreRank` handling for consistent rank labeling.
- 2026-04-02: Removed stale `Game Over` local-highscore rendering/coins/bags wiring that was overriding the new online-left panel and causing mismatched UI state after the recent leaderboard redesign.
- 2026-04-02: Fixed `Top Scores` player-name resolution for run-based leaderboard entries so uploaded runs now map back to the real player name instead of falling back to generic `Player` labels.
- 2026-04-02: Refined `Game Over` leaderboard presentation: panel titles are now `Online Top Scores` / `Online Top Players`, the board label line (`Jump Easy` etc.) is smaller and centered consistently in both panels, and appended rows were reordered to show `Current Run` before `Your Best`.
- 2026-04-02: Prevented scrollbars during the `READY... / RUN!` launch transition by clamping the pre-run card to the play-area height and forcing overflow hidden while launch mode is active.
- 2026-04-02: Added account-style authentication flow for leaderboard identity via new `/api/auth` endpoint. The sign-in popup now requires both `Player Name` and `Password`, auto-creates a new account when the name does not exist, logs in existing accounts when the password matches, and rejects reused names with wrong passwords.
- 2026-04-02: Switched leaderboard identity persistence from auto-generated guest IDs to authenticated account IDs (`a_...`) so online scores are tied to the signed-in account across devices. Legacy local `p_...` IDs now trigger re-authentication instead of silently posting under old guest identities.
- 2026-04-02: Applied pre-run cosmetic tuning: changed `HRRRA` title color to green, centered `Jump Mode` / `Full Mode` card titles, and switched lock requirement copy to red for stronger visibility.
- 2026-04-02: Refined `Game Over` leaderboard layout: removed per-box `Jump Easy/Hard` labels, moved the active board label to a single centered white line under `Game Over`, and forced both online leaderboard boxes to equal height with top-aligned content so shorter lists leave empty space at the bottom instead of changing card size.
- 2026-04-02: Fixed locked requirement copy color visibility by removing grayscale filtering from locked pre-run cards and forcing the lock-note text to a strong red, so `Locked - Reach ...` remains clearly red on real screens.
- 2026-04-02: Fixed Android online auth/leaderboard API targeting for Capacitor builds. The app was resolving `window.location.origin` to `http://localhost`, so auth and score requests failed on Android even though the same flows worked on Vercel; localhost-style runtime origins now fall back to `https://hrrra.vercel.app` for both `/api/auth` and `/api/highscore`.
...
- 2026-04-02: Changed skin discovery pickups to spawn on the right-edge platform instead of the lowest elevator, so collectible skin reveals now follow the same incoming-platform presentation as other platform pickups.
- 2026-04-02: Added a standing workflow note for local Python usage: prefer `py.exe` or the direct `C:\\Program Files\\Python313\\python.exe` path instead of assuming the WindowsApps `python` alias works.
- 2026-04-02: Replaced the separate pre-run `Easy` / `Hard` buttons with one pill-shaped animated difficulty toggle. It now flips between a large green active side and a smaller grey inactive side, and locked `Hard` attempts show the unlock requirement directly across the whole toggle instead of only in a small note below.
- 2026-04-02: Tightened the standing prompt-logging workflow: every user message must be appended to `prompts.md` first, and only after that may any response, analysis, or implementation work begin.
- 2026-04-02: Added a new pre-run `Scores` page between `Badges` and `Sign In`. It loads four `Top Players` boards (`Jump Easy`, `Jump Hard`, `Full Easy`, `Full Hard`) and shows the top 15 unique players per board using the same one-entry-per-player leaderboard data as the right-side `Game Over` panel.
- 2026-04-03: Renamed the new pre-run scores header to `Top Score`, removed the explanatory subtitle, and centered the heading visually while keeping the `Back` button accessible on the left.
- 2026-04-03: Bumped Android/store versioning to `versionCode 14` / `versionName 1.0.13` and refreshed shipped version metadata for a new Play upload containing the new pre-run `Scores` page and related UI refinements.
- 2026-04-03: Simplified mobile-facing UI copy on the main pre-run selection screen: `Jump Mode` now says `Tap To Jump`, `Full Mode` no longer shows desktop-control text, and the phone mockup is centered in its card. On `Game Over`, removed the desktop restart prompt and centered the score/time block more cleanly in the middle column.
- 2026-04-03: Expanded the standing release workflow for `prepare aab`: it now explicitly means the full store-release path, including committing the intended release state, pushing `main`, aligning all version metadata, building the `.aab`, and verifying that the live Vercel `version.json` already exposes the new version so older installed apps can actually see the update on launch.
- 2026-04-03: Added `Question Coin` `Win percent` and `Lose percent` admin fields for every level/mode/difficulty configuration. The gamble result is no longer hardcoded to `+200% / -50%`; those values are now tunable in admin, with defaults kept at `200 / 50` so existing behavior stays unchanged until edited.
- 2026-04-03: Reworked level progression goals from absolute total-score thresholds to per-level required score amounts. The admin field is now labeled `Level Goal Score`, the briefing still shows `Finish Level with XXXX score.`, and `XXXX` is computed as `score carried into the level + required score for that level`, so big late `Question Coin` swings affect the current level but no longer auto-trivialize multiple following levels.
- 2026-04-03: Prepared the Level 1-style gameplay sound folder structure for Levels `2-5` as well. Runtime level audio defaults now automatically point each level to its own `assets/levelX/sound/lX-...` files, and Levels `2-5` were populated with cloned copies of the current Level 1 files as placeholders so those sounds can be replaced gradually without more code changes.
- 2026-04-03: Fixed per-level audio fallback so older admin storage with blank level-audio path fields can no longer override and mute the new Level `2-5` default sound routing. Empty stored audio strings are now ignored, letting the generated `assets/levelX/sound/lX-...` defaults play as intended.
- 2026-04-04: Renamed the two player-facing modes to `Jump Classic` and `Jump Advanced` across the runtime UI. The start screen cards, HUD badges, lock text, online board labels, and relevant admin labels now use the new names, and the pre-run controls copy was simplified to mobile-only wording with no `Mobile:` headings.
- 2026-04-04: Added two new pre-run entry pages on the main selection screen: `Rules` on the left and `Settings` on the right. `Rules` now holds the basic gameplay mechanics / pickup overview that used to live on the detail page, while `Settings` introduces player-facing `Toggle Sound` and `Toggle Music` controls backed by the existing global audio flags so SFX and music can be disabled everywhere, including pre-run, gameplay, level-finished, badges, and game-over screens.
- 2026-04-04: Expanded the new `Rules` page with the missing `Slow` pickup and a visible `Skin Unlock` entry so the rules overview now better reflects the mechanics players can actually encounter.
- 2026-04-04: Refined the `Rules` `Slow` icon so it now visually matches the in-game pickup more closely: yellow diamond sign with the `SLOW / DOWN` look instead of the earlier generic placeholder.
- 2026-04-04: Added a new `Credits` button between `Rules` and `Settings` on the main pre-run top bar. It opens a dedicated credits page with the first-pass project/authorship/tester text supplied for this build.
- 2026-04-04: Reworked `Slow` into a timed effect instead of a permanently compounding slowdown. It now reduces speed by the configured `Slow down by %` only for `Duration sec` (default `10s`), while score-based acceleration keeps progressing underneath; the `Slow` admin section now includes that new duration field for every level/mode/difficulty setup.
- 2026-04-04: Reworked `Question Coin` so the `+ / -` roll no longer auto-resolves after a fixed timer. The overlay now keeps alternating until the player taps to stop it, shows a clear `Tap to stop` prompt, then reveals the locked result briefly before gameplay resumes.
- 2026-04-04: Polished the latest pre-run and `Question Coin` presentation details: the compact `START` CTA no longer breaks into stacked letters on local/mobile layouts, and the tap-stop overlay spacing was adjusted so the `Tap to stop` hint and `+ / -` symbol no longer overlap.
- 2026-04-05: Fixed Android `Question Coin` stop input by routing taps from the fullscreen mobile touch-control zones to the same confirm path as canvas taps. Added a `5s` safety fallback so the roll also auto-stops if no tap arrives.
- 2026-04-05: Tightened several pre-run mobile layouts. `Settings` now uses two compact row toggles instead of oversized cards, `What's New` now scrolls when its content exceeds the viewport, and long-term badges such as lifetime score/coins now show current collected progress in parentheses right inside each target line.
- 2026-04-05: Reduced the new top-bar entry buttons (`Rules`, `Credits`, `Settings`) by about one third and tightened the gaps around them so the main pre-run hub fits better on shorter mobile screens.
- 2026-04-05: Fixed the real gameplay config path so level-specific audio overrides now apply during live Level 2-5 runs as well, restoring each higher level's music and SFX instead of falling back to the wrong/default audio set. Also changed the compact `Settings` view to place the two toggle panels side by side when there is enough horizontal space.
- 2026-04-05: Fixed hero skin carry-over between levels. Continuing from one level to the next now preserves the player's currently selected skin instead of letting the global/default skin overwrite it during level config reload.
- 2026-04-05: Fixed the `Shield Teleporter` badge goal line so it now also shows the current collected shield-teleport count in parentheses, just like the other long-term progression badges.
- 2026-04-05: Added a new design document [`economy.md`](c:/-_WeB_-/Hrrra/economy.md) to capture the current proposal for the future shared coin wallet, Shop system, score exchange, admin Shop section, and planned Game Over coin summary.
- 2026-04-05: Refined [`economy.md`](c:/-_WeB_-/Hrrra/economy.md) with an updated phased rollout: `Game Over` coin summary moves into the first implementation step, admin defines the score-to-coin ratio, and the actual exchange action belongs in the player-facing `Shop` page.
- 2026-04-05: Started the first live economy implementation step on branch `nakupy`: added persistent coin wallet storage, wired collected run coins into that wallet on `Game Over`, added `Game Over` coin/wallet summary lines, and introduced a first `Shop` admin section with economy summary plus baseline tuning fields such as `Score Needed Per 1 Coin`.
- 2026-04-05: Added the first player-facing `Shop` page on branch `nakupy`. Players can now see wallet balance and persistent score, exchange score into coins using the admin-defined ratio, buy `Krob` as the first test shop skin, and see a placeholder card for a future purchasable special level.
- 2026-04-05: Removed the duplicate center-column `Shop` button and kept `Shop` only in the main top bar. Added the first live `continue` purchase flow on `Game Over`: exactly one continue can be bought per run, admin now controls both continue coin price and granted lives, and final run score/coin rewards are only committed when the run really ends so a continue can no longer double-count rewards.
- 2026-04-05: Refined the new `Game Over` continue offer so it stays visible even when the wallet is too small. In that case the screen now shows `Not enough coins to purchase a continue.`, keeps `End Run` active, and renders the `Continue` button greyed out instead of hiding the whole continue option.
- 2026-04-05: Tightened pickup unlock presentation so newly available coins and other platform-based pickups now begin appearing from the right edge of the screen instead of suddenly showing up on already-visible mid-screen platforms. Existing elevator coins that were already inside the current view are also suppressed on first unlock so the first post-unlock coin appearances come from newly arriving right-side content.
- 2026-04-05: Bumped Android/store versioning to `versionCode 19` / `versionName 1.0.18` and updated shipped version metadata for a new Play upload containing the first Shop/economy flow, single-use paid continue, and right-edge pickup unlock presentation fixes.
- 2026-04-05: Added three new admin-configurable economy badge series: `Banger` for exchanged coins, `Big Spender` for total coins spent across the shop economy, and `Unkillable Custommer` for paid continues used. They appear in the same badge/admin systems as the existing bronze/silver/gold badge sets.
- 2026-04-05: Adjusted inline badge progress hints so the current-value parentheses are shown only for locked tiers. Once a tier is already unlocked, the goal line now stays clean without the extra `(current)` progress text.
- 2026-04-05: Bumped Android/store versioning to `versionCode 20` / `versionName 1.0.19` and updated shipped version metadata for a new Play upload containing the latest mobile layout tightening plus the locked-only badge progress hint behavior.
- 2026-04-06: Fixed Game Over continue purchase logic to use Wallet After Run preview instead of only the pre-run wallet balance, removed duplicate (+coins) suffix from Wallet After Run, and matched End Run button width to Continue.
- 2026-04-06: Reworked continue flow into a life-selection overlay. Continue now opens a modal where the player chooses how many lives to buy, sees unit and total price, can only confirm affordable selections, and can cancel back to Game Over.
- 2026-04-06: Swapped the Game Over continue button row above the continue cost text and added a 2-second life-loss invulnerability window for top death zone, projectiles, and blocker after losing a life. Bottom death zone remains instantly fatal.
- 2026-04-06: Added a new [`gfx2.md`](c:/-_WeB_-/Hrrra/gfx2.md) design/implementation document and shipped the first `GFX2` start-screen scaffold behind a new global admin `Start screen style` toggle. `GFX2` uses the new crossroads art with clickable sign hotspots for `Classic`, `Advanced`, `Badges`, `Settings`, and `Scores`, plus drifting cloud buttons for `Rules`, `Credits`, and `Shop`, while the original `GFX1` screen remains available as the default fallback.
- 2026-04-06: Split [`assets/temp/3cloud.png`](c:/-_WeB_-/Hrrra/assets/temp/3cloud.png) into three transparent `GFX2` cloud assets and replaced the temporary pill-style cloud buttons with fixed-position illustrated clouds for `Rules`, `Credits`, and `Shop`. This keeps the current `GFX2` sky layout closer to the final art direction while preserving the same navigation actions.
- 2026-04-06: Reworked `GFX2` start-screen layering so the sky is now a separate blue background, the new cloud buttons sit on their own middle layer, and the crossroads/landscape render above them via [`assets/crossing-foreground.png`](c:/-_WeB_-/Hrrra/assets/crossing-foreground.png). This lets clouds pass visually behind the signpost instead of covering it.
- 2026-04-06: Added the first `GFX2` figure arrival animation. The new [`assets/Figure/`](c:/-_WeB_-/Hrrra/assets/Figure) character now walks in over `2` seconds from the lower-left path and settles near the crossroads center, with responsive size scaling so the placement stays aligned on desktop and mobile.
- 2026-04-06: Refined the `GFX2` figure path anchor and route to follow the marked lower-left road more closely. The tracked point now sits lower and slightly forward on the figure base instead of using the sprite center, and the run no longer switches to the front-facing `05...` frame at the end.
- 2026-04-06: Replaced the simple 4-point `GFX2` figure arrival curve with a multi-point path interpolation tuned against the marked road reference image, so the figure now follows the intended thicker left-hand path with a more accurate bend into the crossroads.
- 2026-04-06: Added a temporary `GFX2` in-scene path debug overlay. The start screen now draws the current figure route as a red polyline with numbered control points directly over the crossroads art so figure-path tuning can be done against the live scene instead of guessing from screenshots.
- 2026-04-06: Changed the temporary `GFX2` figure path playback from smoothed spline interpolation to exact polyline interpolation. The live figure now follows the same straight segment path shown by the debug overlay, so planned and actual route positions can be compared 1:1.
- 2026-04-07: Replaced the temporary `GFX2` moving-figure/debug-path setup with baked entrance background frames from [`assets/gfx2/entrance/`](c:/-_WeB_-/Hrrra/assets/gfx2/entrance). Opening the `GFX2` start screen now plays frames `01-08` over `1s` and then holds the last frame as the static background scene.
- 2026-04-07: Added a baked `Classic` transition for the `GFX2` start screen using [`assets/gfx2/classic/`](c:/-_WeB_-/Hrrra/assets/gfx2/classic). Clicking `Classic` now plays frames `01-07` over `1s` and only after the final frame opens the Classic pre-run screen.
- 2026-04-07: Added a baked `Advanced` transition for the `GFX2` start screen using [`assets/gfx2/advance/`](c:/-_WeB_-/Hrrra/assets/gfx2/advance). Clicking `Advanced` now plays frames `01-06` over `1s` and only after the final frame opens the Advanced pre-run screen, while the existing full-mode lock gate remains in place.
- 2026-04-07: Added a baked `Scores` transition for the `GFX2` start screen using [`assets/gfx2/scores/`](c:/-_WeB_-/Hrrra/assets/gfx2/scores). Clicking `Scores` now plays frames `01-05` over `1s` and only after the final frame opens the online scores screen.
- 2026-04-07: Added a baked `Badges` transition for the `GFX2` start screen using [`assets/gfx2/badges/`](c:/-_WeB_-/Hrrra/assets/gfx2/badges). Clicking `Badges` now plays frames `01-08` over `1s` and only after the final frame opens the badges screen.
- 2026-04-07: Fixed `GFX2` cloud button layering so the `Rules`, `Credits`, and `Shop` clouds stay visible above the baked background frame sequences instead of briefly flashing only before the foreground image finishes loading.
- 2026-04-07: Tightened the `GFX2` cloud button layout so the clouds scale down more conservatively with the scene and the `Credits` cloud sits further left, reducing overlap with the crossroads hill, signpost, and the `Advanced` hotspot.
- 2026-04-07: Reworked the mobile/small-viewport pre-run shell into a true fullscreen overlay. On Android-sized screens the start screen now uses the whole viewport with an opaque background, so the underlying HUD/admin no longer shows through behind the pre-run scene.
- 2026-04-07: Converted the baked `GFX2` frame sequences from `.png` to compressed `.jpg`, reducing the total sequence payload from about `69 MB` to about `8.5 MB` to improve first-load decode/caching behavior and reduce visible frame flicker on web and Android.

- 2026-04-07: Added the new Shop house/sign artwork from [`assets/temp/entrance/entrance_shop.jpg`](c:/-_WeB_-/Hrrra/assets/temp/entrance/entrance_shop.jpg) into every baked GFX2 entrance frame so the entrance animation and held final frame now show the shop marker consistently in the same position.

- 2026-04-07: Propagated the same Shop house/sign artwork from [`assets/temp/entrance/entrance_shop.jpg`](c:/-_WeB_-/Hrrra/assets/temp/entrance/entrance_shop.jpg) to the baked GFX2 classic, advance, scores, badges, and shop frame sequences as well, keeping the shop marker aligned in the same position across all crossroads transition animations.
- 2026-04-07: Upgraded GFX2 frame preloading from fire-and-forget image requests to a persistent load/decode cache. The crossroads entrance and exit animations now wait until their frame sets are fully loaded before playing, which is intended to remove the first-run flicker seen before the browser cache warmed up.
...
- 2026-04-07: Added the current app version badge to the top-right corner of the Credits screen and wired it to the shared `version-info.js` metadata so Credits always shows the active release version automatically.
- 2026-04-07: Rebuilt only the baked `Badges` and `Shop` transition frame sequences using the correct `assets/shop/shop.png` source, keyed the white background to transparency, and placed it from the exact `assets/shop/shop_full.png` reference bounds so the smaller house/sign patch interferes less with the passing character.
- 2026-04-07: Added the correct baked shop-house graphic from `assets/shop/shop.png` to every `GFX2` `_back` sequence frame (`classic_back`, `advance_back`, `badges_back`, `scores_back`, `shop_back`, `settings_back`) and changed `GFX2` back navigation so returning from Classic, Advanced, Badges, Scores, Shop, and Settings now plays the matching 1-second `_back` animation instead of replaying the default entrance.
- 2026-04-07: Restored touch scrolling on the `Classic`, `Advanced`, and `Badges` pre-run pages by making the scrollable pre-run stages (`details`, `badges`, and related overview pages) explicitly use `overflow-y: auto`, touch pan-y, and momentum scrolling inside the fullscreen pre-run card.
- 2026-04-07: Changed the last-life `End Run` action so it now finalizes the run and returns straight to the pre-run screen instead of showing a second redundant Game Over confirmation screen; if new badge rewards were earned, they still play first and then return to pre-run.
- 2026-04-07: Added an explicit workflow rule that every new `.aab` preparation must bump the release version first, then bumped the release to `versionCode 25` / `versionName 1.0.24` and refreshed the shipped update metadata.
- 2026-04-08: Reworked the `GFX2` Shop screen into a dedicated interior layout using [`assets/gfx2/shop_inside.png`](c:/-_WeB_-/Hrrra/assets/gfx2/shop_inside.png), with responsive hotspot buttons mapped to the in-image controls/items, a chalkboard info panel (`Total Points`, `Coin Wallet`, selected item, cost), left/right coin piles wired to `Buy 1` / `Buy 10`, a shared `Buy` action, and placeholder selections for `New Level` plus character/chest items.
