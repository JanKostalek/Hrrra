...
- 2026-05-25: Bumped the release metadata to 1.0.105 / versionCode 105 for the next Store build while keeping the current in-app `What's New` text unchanged.
- 2026-05-25: Clarified the admin-toggle workflow rules so `odkryj admin` now explicitly shows and re-enables all prerun admin entry points, while `skryj admin` hides and disables them, with the state persisted in `hrrra_admin_toggle_visible_v1`.
- 2026-05-25: Added the bundled default admin snapshot from `hrrra-settings_defualt.json` through `hrrra-settings-default.js`, and made reset/default plus first-launch users seed that export into admin storage.
- 2026-05-25: Wired all prerun admin entry points in `game.js` and `index.html` to one local visibility toggle, added `window.HrrraDebug.showAdminToggle()` / `hideAdminToggle()` helpers for `skryj admin` and `zobrazit admin`, and hid the prerun admin buttons by default.
...
- Store build now hides the prerun admin button, with SHOW_ADMIN_TOGGLE in game.js as the local reveal switch.
