...
priprav aab
a) v online verzi pri mouse hover se objeví po straně posuvník, který říká, že obraz / text lze posouvat. Kdyz myší posouvám, vidím ten posuvník, jak se pohybuje. ovsem v android verzi (ani emulator ani telefon) se posuvník nezobrazuje ani pri samotném posouvání obrazu. Není tedy na první pohled jasné, že tam je víc textu nez na te viditelné části. můžeš tam "natvrdo" přidat tenký pousuvník? U stránek, kde je text i mimo textové pole a je možno jej posunout
Zkontroluj, odkud se v pre-run obrazovce načítá grafika pro Skin05 a Skin06, a oprav to tak, aby se po koupi zobrazovaly jejich vlastní selected/unselected soubory z `assets/skins/Skin05` a `assets/skins/Skin06` místo question-mark placeholderu z layoutu.
Ověř, že classic i advanced pre-run sloty používají sdílený resolver do `assets/gfx2/classic/layout/skinXX_locked|selected|unselected.jpg` a že Skin01 je jediný slot bez locked varianty, protože je vždy otevřený.
Sjednoť i velikost preview obrázků ve skin gridu, aby všechny skiny používaly stejný rozměr jako slot 6 a Skin01 už nebyl renderovaný větší než ostatní.
Srovnej i classic/advanced slot artwork na pre-run obrazovce tak, aby všechny sloty 1 až 6 používaly stejnou velikost jako slot 6 a už nebyly per-slot odlišné rozměry v CSS.
2026-05-14: Implemented Cube as Skin07, linked its selection to Modern Visuals, added a Cube shop item priced at 5000 coins, and updated pre-run visuals to show `skin07_unselected.png` / `skin07_selected.png` when unlocked.
2026-05-14: Fixed the pre-run skin grid layout so the Cube slot stays visible in the 3-column skin panel instead of being pushed out by auto-fit sizing.
2026-05-14: Added a red debug outline and tighter card sizing to expose the Cube slot in the pre-run skin panel.
