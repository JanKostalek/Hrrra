...
- Continue rework: after death, Continue now opens a small life-purchase overlay instead of directly granting a fixed life bundle. The player selects how many lives to restore up to the current mode maximum, sees the total coin cost, and can only buy an affordable selection. Continue can still be used only once per run.
- After a non-final life loss from top death zone, projectile, or blocker, the player gets a 2-second protection window against additional life loss from those same sources. Bottom death zone is excluded and still kills instantly.
